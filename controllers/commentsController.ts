...
const updateComment = asyncHandler(async (req, res) => {
  ...
  const updatedComment = await updateThisComment(
    req.user.id,
    formData.id,
    formData.content,
  );
  ...
  if (req.io) {
    req.io
      .to(`post:${updatedComment.postid}:comments`)
      .emit("comment:updated", {
        ...
        ...(updatedComment.commentid
          ? { parentid: updatedComment.commentid }
          : {}),
      });
  }
...
const deleteComment = asyncHandler(async (req, res) => {
  ...
  if (req.io) {
    ...
    req.io
      .to(`post:${deletedComment.postid}:comments`)
      .emit("comment:deleted", {
        id: deletedComment.id,
        postid: deletedComment.postid,
        ...(deletedComment.commentid
          ? { parentid: deletedComment.commentid }
          : {}),
...
const getComments = asyncHandler(async (req, res) => {
  ...
  const foundComments = await getThisCommentComments(
    formData.id,
  ...
  res
    .status(200)
    .json({
      comments: foundComments.map(({ _count, ...val }) => ({
        ...val,
        likesCount: _count.likes,
        ownCommentsCount: _count.ownComments,
      })),
...
const changeLike = asyncHandler(async (req, res) => {
  ...
  const changedComment = await changeCommentLike(
    req.user.id,
    formData.id,
  ...
  if (req.io) {
    req.io
      .to(`post:${changedComment.postid}:comments`)
      .emit("comment:updated", {
        type: "likes",
        postid: changedComment.postid,
        id: changedComment.id,
        likes: _count.likes,
        ...(changedComment.commentid
          ? { parentid: changedComment.commentid }
          : {}),
      });
  }
...
export { updateComment, deleteComment, getComment, changeLike, getComments };
