...
const deleteRequest = asyncHandler(async (req, res) => {
    ...
    if (req.io) {
        if (req.user.id === deletedRequest.senderid) {
            req.io.to(`self:${deletedRequest.targetid}`).emit("request", {action: "REMOVE", data: {id: deletedRequest.id, userid: deletedRequest.targetid, myid: deletedRequest.targetid}});
        } else if (req.user.id === deletedRequest.targetid) {
            req.io.to(`self:${deletedRequest.senderid}`).emit("request", {action: "REMOVE", data: {id: deletedRequest.id, userid: deletedRequest.targetid, myid: deletedRequest.senderid}});
        }
    };
