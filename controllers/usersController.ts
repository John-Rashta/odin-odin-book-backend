...
const stopFollowing = asyncHandler(async (req, res) => {
  ...
  if (req.io) {
    ...
    req.io.to(`self:${req.user.id}`).emit("follows", {action: "REMOVE", id: possibleUser.id})
...
const getMyFollows = asyncHandler(async (req, res) => {
  ...
  const myFollows = await getMyFollowships(req.user.id, "followers", getTakeAndSkip({amount: formData.amount, skip: formData.skip}), "yes");
