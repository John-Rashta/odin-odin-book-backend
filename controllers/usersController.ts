...
const stopFollowing = asyncHandler(async (req, res) => {
  ...
  if (req.io) {
    req.io
      .to(`self:${possibleUser.id}`)
      .emit("followers", { action: "REMOVE", id: req.user.id });
    req.io
      .to(`self:${req.user.id}`)
      .emit("follows", { action: "REMOVE", id: possibleUser.id });
    req.io
      .to(`user:${possibleUser.id}`)
      .emit("user:updated", {
        type: "followers",
        newCount: possibleUser._count.followers,
        id: possibleUser.id,
      });
  }
...
const searchUsers = asyncHandler(async (req, res) => {
  ...
  if (isUUID(formData.user)) {
    const possibleUser = await getThisUser(
      formData.user,
    ...
    res
      .status(200)
      .json({ users: [{ ...rest, followerCount: _count.followers }] });
    return;
  }
...
const getMyFollowers = asyncHandler(async (req, res) => {
  ...
  const myFollowers = await getMyFollowships(
    req.user.id,
    "follows",
    getTakeAndSkip({ amount: formData.amount, skip: formData.skip }),
    "yes",
...
const getMyFollows = asyncHandler(async (req, res) => {
  ...
  const myFollows = await getMyFollowships(
    req.user.id,
    "followers",
    getTakeAndSkip({ amount: formData.amount, skip: formData.skip }),
    "yes",
...
const getUserPosts = asyncHandler(async (req, res) => {
  ...
  const allPosts = await getThisUserPosts(
    formData.id,
    getTakeAndSkip({ amount: formData.amount, skip: formData.skip }),
  );
...
const getMyFeed = asyncHandler(async (req, res) => {
  ...
  const myFeed = await getUserFeed(
    req.user.id,
    getTakeAndSkip({ amount: formData.amount, skip: formData.skip }),
  );
