...
const validateRequest = [
  body("id").isUUID().withMessage("Must be an UUID."),
  body("type").isIn(["FOLLOW"]).withMessage("Must be a valid type."),
];

const validateSearch = [
  query("user").custom((value) => {
    return isUUID(value) || isAlphanumeric(value);
  }),
];

const validatePost = [
  body("content").optional({ values: "falsy" }).notEmpty().isString().trim(),
];

const validateLikeType = [
  body("action").isIn(["ADD", "REMOVE"]).withMessage("Must be a valid action."),
];
